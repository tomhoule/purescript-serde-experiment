-- Rust serde-json compatible encoding/decoding:

module Data.Argonaut.Generic.Serde
  ( encodeJson
  , decodeJson
  , options
  , userEncoding
...
-- | Options for Serde compatible encoding/decoding.
options :: Options
options = Options {
...
-- | with Serde options. The encoded data will be compatible with Serde,
-- | if Serd default options are used.
encodeJson :: forall a. (Generic a) => a -> Json
encodeJson = genericEncodeJson options

-- | Decode `Json` representation of a value which has a `Generic` type
-- | with Serde options. Data from Haskell, with Serde default options can be
-- | decoded with gSerdeDecodeJson.
